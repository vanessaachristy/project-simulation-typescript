...
import { getAllUsageByPhoneNumber } from "../services/usage.service";
import { ApiResponse, Usage, UsageDetails } from "../types";
import { getPlanById } from "../services/plan.service";


export const getBillingHandler = async (request: FastifyRequest, _reply: FastifyReply) => {
    ...
    const planInfo = await getPlanById(usages[0].planId);
