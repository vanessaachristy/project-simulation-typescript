...
import { getSubscriberId } from '../services/subscriber.service';
import { insertUsageData } from '../services/usage.service';

const filePath = path.join(__dirname, '../../data/usage.csv');


export const importCSVHandler = async (request: FastifyRequest, _reply: FastifyReply) => {
    const filePath = path.join(__dirname, '../../data/usage.csv');
    ...
    for (const row of csvData) {
        ...
        try {
            ...
            const subscriberId = await getSubscriberId(phone_number, plan_id);

            // Insert usage data
            const result = await insertUsageData(subscriberId, stringDate, Number(usage_in_mb));
